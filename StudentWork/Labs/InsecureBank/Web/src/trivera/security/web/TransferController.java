...
public class TransferController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (target == null || target.isEmpty()) {
			genBadIdPage("Please enter a valid account number to transfer to.",
					request, response);
			return;
		};		
		...
		Account targetAccount = store.getAccount(target);
		
		// If no match, no more
		if (targetAccount == null) {
			genBadIdPage(target + " is not a valid account number.",
					request, response);
			return;
		}
