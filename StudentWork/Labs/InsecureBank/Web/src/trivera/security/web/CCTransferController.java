...
public class CCTransferController extends HttpServlet {
	/**
	 * 
	 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (source == null || source.isEmpty()) {
			genBadIdPage("Please enter a valid account number to transfer from.",
					request, response);
			return;
		};			
		...
		Account sourceAccount = store.getAccount(source);
		
		// If no match, no more
		if (sourceAccount == null) {
			genBadIdPage(source + " is not a valid account number.",
					request, response);
			return;
		}	
		...
		request.setAttribute("targetcc", (String)request.getParameter("target"));
		request.getRequestDispatcher("JSPs/CCTransferResults.jsp").forward(
				request, response);
	}
